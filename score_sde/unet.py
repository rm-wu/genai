...
class ResidualBlock(eqx.Module):
    ...
    def __call__(
        self, x: Array, time_embedding: Optional[Array] = None
    ) -> Array:
        x = self.conv(x)
        x = self.norm(x)
        if exists(time_embedding) :
            x = x + time_embedding
        x = self.act(x)
        return x
...
class ResNetBlock(eqx.Module):
    ...
    def __call__(self, x: Array, time_embedding: Optional[Array] = None) -> Array:
        if exists(time_embedding) and exists(self.time_mlp) and exists(self.time_act):
            time_embedding = self.time_mlp(time_embedding)
            time_embedding = self.time_act(time_embedding)
            time_embedding = jnp.reshape(time_embedding, (-1, 1, 1))
            

        h = self.block1(x=x, time_embedding=time_embedding)
        h = self.block2(x=h)
        x = self.res_conv(x)
        x = h + x
        return x
...
class Attention(eqx.Module):
    ...
    def __call_(self, x: Array) -> Array:
        pass
